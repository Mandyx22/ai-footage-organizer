...
export default function Library() {
  ...
  const similarInput = useMemo(
    () => ({ clipId: focusedClipId ?? 0, dimension: similarDimension }),
    [focusedClipId, similarDimension]
  );
  const library = trpc.footage.sampleList.useQuery();
  const searched = trpc.footage.sampleSearch.useQuery(searchInput, {
  ...
  return (
    <>
      <SketchShell active="library" onUpload={() => setUploadOpen(true)}>
        <div className="mx-auto max-w-[1560px] px-4 pb-12 pt-8 sm:px-7 lg:px-10">
          <section className="grid gap-7 xl:grid-cols-[minmax(0,1fr)_300px]">
            <div>
              <div className="mb-5 rounded-2xl border-[1.5px] border-[#2c2922]/55 bg-[#fff1ba] p-4 shadow-[2px_2px_0_rgba(44,41,34,.13)]">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <LibrarySourceStatus mode="sample" />
                    <h1 className="mt-1 font-hand text-4xl font-bold leading-none">
                      Try the product before making it yours.
                    </h1>
                    <p className="mt-2 max-w-2xl text-xs leading-5 ink-muted">
                      These eight fictional clips demonstrate search, visual
                      notes, selection, and creative reasoning. They are not
                      your uploads, and no action here changes your personal
                      workspace.
                    </p>
                  </div>
                  <Link
                    href="/docs#sample"
                    className="inline-flex shrink-0 items-center gap-1 rounded-lg border border-[#2c2922]/40 bg-[#fffdf7] px-2.5 py-1.5 text-[11px] font-semibold shadow-sm"
                  >
                    What is sample mode? <ChevronRight className="size-3" />
                  </Link>
                </div>
              </div>
              <div className="relative max-w-2xl">
                ...
                <input
                  ...
                  placeholder="Search a memory — “quiet blue night shots”"
              ...
              {!showSimilar && searchQuery.trim().length > 1 && (
                <div className="note-green mt-5 flex items-center gap-2 rounded-xl border-[1.5px] border-[#2c2922]/48 px-3 py-2.5 text-xs shadow-[2px_2px_0_rgba(44,41,34,.12)]">
                  ...
                  <span>
                    <b>{visibleClips.length} matches</b> for “
                    {searchQuery.trim()}”. Each card shows its matching clues.
                  </span>
                </div>
              )}
              ...
              {!visibleClips.length && (
                <div className="paper-panel mt-4 rounded-2xl px-6 py-14 text-center">
                  <Archive className="mx-auto size-6 ink-muted" />
                  <p className="mt-3 text-sm font-semibold">
                    Nothing surfaced just yet.
                  </p>
                  <p className="mt-1 text-xs ink-muted">
                    Try a feeling, colour, subject, time of day, or a simpler
                    phrase.
                  </p>
                </div>
              )}
            ...
            <aside className="space-y-5 xl:pt-2">
              {focusedClip && (
                <div className="tape note-yellow rounded-2xl border-[1.5px] border-[#2c2922]/58 p-4 shadow-[3px_3px_0_rgba(44,41,34,.14)]">
                  <div
                    className={cn(
                      "relative aspect-[16/10] overflow-hidden rounded-xl border-[1.5px] border-[#2c2922]/55 bg-gradient-to-br",
                      gradients[Math.abs(focusedClip.id) % gradients.length]
                    )}
                  >
                    {(focusedClip.thumbnailUrl ??
                      demoImages[focusedClip.id]) && (
                      <img
                        src={
                          focusedClip.thumbnailUrl ?? demoImages[focusedClip.id]
                        }
                        alt=""
                        onError={event =>
                          hideBrokenImageElement(event.currentTarget)
                        }
                    ...
                    <div className="absolute inset-x-3 bottom-3 flex items-center justify-between">
                      <span className="rounded-md bg-[#fffdf7]/90 px-1.5 py-1 font-mono text-[9px] shadow-sm">
                        {formatDuration(focusedClip.durationMs)}
                      </span>
                      <button
                        onClick={() => setShowSimilar(true)}
                        className="rounded-md border border-[#2c2922]/55 bg-[#fffdf7]/90 px-2 py-1 text-[10px] font-semibold shadow-sm hover:bg-white"
                      >
                        Find similar
                      </button>
                    </div>
                  ...
                  <div className="mt-4 flex items-start justify-between gap-2">
                    <div>
                      <p className="font-mono text-[10px] ink-muted">
                        CLIP NOTES · SAMPLE
                      </p>
                      <p className="mt-1 text-sm font-bold">
                        {focusedClip.fileName}
                      </p>
                    </div>
                    <button
                      onClick={() => toggleSelection(focusedClip.id)}
                      className={cn(
                        "grid size-7 place-items-center rounded-lg border-[1.5px]",
                        isSelected(focusedClip.id)
                          ? "border-[#2c2922] bg-[#f4ad89]"
                          : "border-[#2c2922]/45 bg-[#fffdf7] ink-muted hover:text-[#2c2922]"
                      )}
                    >
                      <Check className="size-3.5" />
                    </button>
                  </div>
                  <p className="mt-3 text-xs leading-5 ink-muted">
                    {focusedClip.description}
                  </p>
                  <div className="mt-4 flex flex-wrap gap-1.5">
                    {[
                      ...focusedClip.mood,
                      focusedClip.shotType,
                      ...focusedClip.colors.slice(0, 1),
                    ].map(tag => (
                      <span
                        key={tag}
                        className="rounded-md border border-[#2c2922]/25 bg-[#fffdf7]/65 px-1.5 py-1 text-[10px]"
                      >
                        {tag}
                      </span>
                    ))}
              ...
              <div className="paper-panel rounded-2xl p-4">
                <p className="font-hand text-xl font-bold">Sample selection</p>
                <p className="mt-1 text-xs leading-5 ink-muted">
                  Circle sample clips to try the selection and Ask My Footage
                  flow. Nothing is saved to your Workspace.
                </p>
                <div className="mt-4 flex items-center justify-between">
                  <span className="rounded-md bg-[#dcefdc] px-2 py-1 font-mono text-[10px]">
                    {selectedIds.length} circled
                  </span>
                  <Link
                    href="/ask"
                  ...
                  >
                    Ask about sample <Sparkles className="size-3" />
                  </Link>
                </div>
              </div>
      ...
      <UploadFootageDialog open={uploadOpen} onOpenChange={setUploadOpen} />
    </>
  );
}
    ...
    >
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button
              onClick={event => {
                event.stopPropagation();
                toast.info(
                  "Pick a clip to see its notes or find similar material."
                );
              }}
              className="grid size-6 place-items-center rounded-md border border-[#2c2922]/35 bg-[#fffdf7]/90 opacity-0 shadow-sm transition-opacity group-hover:opacity-100"
            >
      ...
      <div className="p-3.5">
        <div className="flex items-center justify-between gap-2">
          ...
          <span className="shrink-0 rotate-1 rounded bg-[#e8eff7] px-1 text-[9px] uppercase tracking-[.12em] ink-muted">
            {clip.time}
          </span>
        </div>
        <p
