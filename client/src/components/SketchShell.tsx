...
import { BookOpenText, Clapperboard, FolderHeart, FolderOpen, Grid2X2, HelpCircle, ImageUp, MessageCirclePlus, Sparkles } from "lucide-react";
import type { ReactNode } from "react";
import { Link } from "wouter";

type ActiveRoute = "myLibrary" | "library" | "collections" | "ask" | "docs";

const workspaceLinks = [
  { id: "myLibrary", label: "My library", href: "/my-library", icon: FolderOpen },
  { id: "library", label: "Sample library", href: "/library", icon: Grid2X2 },
  { id: "collections", label: "Collections", href: "/collections", icon: FolderHeart },
...
export function SketchShell({ active, onUpload, children }: { active: ActiveRoute; onUpload: () => void; children: ReactNode }) {
  ...
  return <div className="app-glow min-h-screen overflow-x-hidden ink">
    ...
    <main className="lg:ml-[226px]">
      <header className="sticky top-0 z-20 border-b-[1.5px] border-[#2c2922]/55 bg-[#f7f3eb]/90 px-4 py-3 backdrop-blur sm:px-7 lg:px-10"><div className="mx-auto flex max-w-[1560px] items-center gap-3"><Link href="/" className="grid size-9 -rotate-3 place-items-center rounded-xl sketch-outline bg-[#f4ad89] lg:hidden"><Clapperboard className="size-4" /></Link><div className="min-w-0 flex-1"><p className="font-hand text-xl font-bold leading-none sm:text-2xl">{active === "myLibrary" ? "My library" : active === "library" ? "Sample library" : active === "collections" ? "Collections" : active === "ask" ? "Ask my footage" : "Documentation"}</p><p className="mt-0.5 hidden font-mono text-[9px] uppercase tracking-[.15em] ink-muted sm:block">{active === "myLibrary" ? "Your uploads · your working material" : active === "library" ? "Explore first · make later" : active === "collections" ? "Gather the beginnings of an edit" : active === "ask" ? "Creative reasoning from selected material" : "How Framefind works"}</p></div><Button onClick={onUpload} className="h-10 -rotate-1 rounded-xl border-[1.5px] border-[#2c2922] bg-[#f4ad89] px-3 text-xs font-bold text-[#2c2922] shadow-[2px_2px_0_#2c2922] hover:bg-[#fac7ae] sm:px-4"><ImageUp className="mr-1.5 size-4" /><span className="hidden sm:inline">Upload footage</span></Button>{!loading && (isAuthenticated ? <button onClick={logout}><Avatar className="size-9 border-[1.5px] border-[#2c2922]/70"><AvatarFallback className="bg-[#dcefdc] text-xs font-bold text-[#2c2922]">{user?.name?.[0]?.toUpperCase() ?? "U"}</AvatarFallback></Avatar></button> : <Button onClick={() => startLogin()} variant="ghost" className="h-10 rounded-xl text-xs ink-muted hover:bg-[#fffdf7] hover:text-[#2c2922]">Sign in</Button>)}</div></header>
      <nav className="border-b border-[#2c2922]/20 bg-[#fffdf7]/65 px-4 py-2 lg:hidden"><div className="flex gap-2 overflow-x-auto">{workspaceLinks.map(item => { const Icon = item.icon; return <Link key={item.id} href={item.href} className={cn("flex shrink-0 items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-[11px]", active === item.id ? "bg-[#f8d9cc] font-semibold" : "ink-muted")}><Icon className="size-3.5" />{item.label}</Link>; })}<Link href="/docs" className={cn("flex shrink-0 items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-[11px]", active === "docs" ? "bg-[#e8eff7] font-semibold" : "ink-muted")}><BookOpenText className="size-3.5" />Docs</Link></div></nav>
      {children}
    </main>
  </div>;
}
