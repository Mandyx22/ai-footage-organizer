...
import { representativeFrame, type UploadJob, uploadRawVideo } from "@/lib/footage";
import { finalizeUploadCompletion, refreshPersonalFootageQueries } from "@/lib/uploadOutcome";
...
export function UploadFootageDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  const processFiles = async (files: File[]) => {
    ...
    for (const file of videoFiles) {
      ...
      setJobs(current => [{ id, fileName: file.name, progress: 7, state: "sampling" }, ...current]);
      try {
      ...
      } catch (error) { setJobs(current => current.map(job => job.id === id ? { ...job, progress: 100, state: "failed", error: error instanceof Error ? error.message : "Upload failed." } : job)); }
    }
