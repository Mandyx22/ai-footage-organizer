...
The MVP treats the creative process as retrieval and discovery. A creator may remember a clip as “blue”, “quiet”, “with friends”, or “good for a transition”, even when its filename is `IMG_4821.MOV`. The product must make those memory cues actionable.
...
| Collections | Turn discovered footage into editable groups. | Create a named collection, save a current selection, and show automatically derived thematic group suggestions. |
...
| Organize | Selects clips and creates a collection. | Persists the named collection and membership. | A potential montage / scene now has a home. |
...
| FR-07 | Collections | An authenticated creator can create an empty collection or create one from a selected group of their own clips. |
...
| --- | --- | --- |
| `clips` | One uploaded footage record. | File identity, media / thumbnail storage references, analysis status, descriptive metadata, and owner. |
| `collections` | A named creator-controlled group. | Owner, name, description, visual accent, and whether it is AI-suggested. |
| `collectionClips` | Membership relationship. | Composite collection / clip key and creation timestamp. |
...
The interface should show AI reasoning as an assistive layer. Search cards expose matching signals, suggested collections label their origin, and subjective fields such as mood are presented as practical creative prompts rather than deterministic judgments.
